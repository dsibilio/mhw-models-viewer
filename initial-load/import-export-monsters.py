...
rootdir = r'D:\UnchunkedMHW\chunk\em'
rootdir_parent = os.path.split(rootdir)[0]
output_rootdir = r'D:\OutputMHW'
excluded_dirs = ['collision', 'data', 'epv', 'ik', 'lookat', 'mot', 'shell', 'sound', 'trace']
texture_suffix = '_BML.PNG'
fallback_texture_suffix = '_BM.PNG'

def get_logical_name(subdir):
    subdir_parent = os.path.split(subdir)[0]
    relative_subdir = str(Path(subdir_parent).relative_to(rootdir))
    return os.path.split(relative_subdir)[0] + "_" + os.path.split(relative_subdir)[1]
...
for subdir, dirs, files in os.walk(rootdir):
    if not any(exclusion in subdir for exclusion in excluded_dirs):
        for file in files:
            if file.replace('.mod3', '') in get_logical_name(subdir):
                ...
                try:
                    ...
                    try:
                        texture_file_path = import_file_path.replace('.mod3', fallback_texture_suffix)
                        out_texture_file_path = output_file_path.replace('.obj', fallback_texture_suffix)
                        shutil.copyfile(texture_file_path, out_texture_file_path)
                    except FileNotFoundError:
